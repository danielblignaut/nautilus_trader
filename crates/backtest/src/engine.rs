...
pub struct BacktestEngine {
    ...
    data: Vec<Data>,
    data_iter: Option<Box<dyn Iterator<Item = Data>>>,
    index: usize,
    iteration: usize,
...
impl BacktestEngine {
    ...
    /// Add a streaming data iterator to the engine.
    ///
    /// When set, `run()` will pull data on-demand from this iterator instead of
    /// using the in-memory `data` Vec. This avoids loading all data into memory.
    pub fn add_data_iterator(&mut self, iter: impl Iterator<Item = Data> + 'static) {
        self.data_iter = Some(Box::new(iter));
    }

    /// Register instrument IDs for data/book tracking without scanning actual data.
    ///
    /// This is needed when using streaming iterators so the engine knows which
    /// instruments and venues to expect without a full data scan.
    pub fn register_data_instruments(&mut self, ids: &[InstrumentId]) {
        for id in ids {
            self.has_data.insert(*id);
            self.add_market_data_client_if_not_exists(id.venue);
        }
    }

    /// Register instrument IDs that will have order book data.
    pub fn register_book_data_instruments(&mut self, ids: &[InstrumentId]) {
        for id in ids {
            self.has_book_data.insert(*id);
        }
    }
    ...
    /// Add a boxed strategy (trait object) to the engine.
    ///
    /// Wraps the `Box<dyn Strategy>` in a [`BoxedStrategy`] which implements
    /// all required concrete traits, then delegates to `add_strategy`.
    pub fn add_strategy_boxed(&mut self, strategy: Box<dyn Strategy>) -> anyhow::Result<()> {
        self.add_strategy(nautilus_trading::strategy::boxed::BoxedStrategy(strategy))
    }
    ...
    pub fn run(&mut self, start: Option<UnixNanos>, end: Option<UnixNanos>) -> anyhow::Result<()> {
        let has_streaming = self.data_iter.is_some();
        anyhow::ensure!(
            !self.data.is_empty() || has_streaming,
            "No data to run backtest"
        );
        ...
        if has_streaming {
            // Streaming mode: pull data from iterator on demand
            let start_ns = start.unwrap_or(UnixNanos::default());
            let end_ns = end.unwrap_or(UnixNanos::from(u64::MAX));
            ...
            let mut iter = self.data_iter.take().unwrap();
            ...
            for data in iter.by_ref() {
                ...
                self.process_data_item(&data, ts, &mut last_ns)?;

                // process_data takes ownership
                self.kernel.data_engine.borrow_mut().process_data(data);

                self.post_process_data_item(ts)?;
            }
        } else {
            // In-memory mode: iterate over self.data vec
    ...
    /// Advance clock and route data to simulated exchange.
    fn process_data_item(
        &mut self,
        data: &Data,
        ts: UnixNanos,
        last_ns: &mut UnixNanos,
    ) -> anyhow::Result<()> {
    ...
    /// Drain deferred events, process exchanges, handle time events.
    fn post_process_data_item(&mut self, ts: UnixNanos) -> anyhow::Result<()> {
