...
impl HyperliquidHttpClient {
    ...
    pub async fn request_account_state(&self, user: &str) -> Result<AccountState> {
        ...
        // Build balances from cross margin summary
        // Always include a USDC balance even if zero to ensure account state is valid
        let balances = if let Some(margin) = &state.cross_margin_summary {
            // total_raw_usd is the USDC collateral balance
            let total_amount = margin.total_raw_usd.to_f64().unwrap_or(0.0);
            // withdrawable is the free balance (optional, default to total if not present)
            let free_amount = margin
                .withdrawable
                .and_then(|w| w.to_f64())
            ...
            vec![AccountBalance::new(
                Money::new(total_amount.max(0.0), usdc),
                Money::new(locked_amount.max(0.0), usdc),
                Money::new(free_amount.max(0.0), usdc),
            )]
        } else {
            // No margin summary, return zero balance
            vec![AccountBalance::new(
                Money::new(0.0, usdc),
                Money::new(0.0, usdc),
                Money::new(0.0, usdc),
    ...
    pub async fn submit_order(
        ...
        // Normalize decimals to ensure consistent serialization for L1 signature
