...
impl SandboxInner {
    ...
    fn process_order_book_deltas(&mut self, deltas: &OrderBookDeltas) {
        if !self.config.orderbook_execution {
            return;
        }
        ...
        if let Some(instrument) = instrument {
            self.ensure_matching_engine(&instrument);
            if let Some(engine) = self.matching_engines.get_mut(&instrument_id) {
                if let Err(e) = engine.get_engine_mut().process_order_book_deltas(deltas) {
                    log::warn!("Failed to process order book deltas: {e}");
                }
            }
        }
    }
...
struct RegisteredHandlers {
    ...
    deltas_pattern: Option<MStr<Pattern>>,
    deltas_handler: Option<TypedHandler<OrderBookDeltas>>,
}
...
impl SandboxExecutionClient {
    ...
    ///
    /// This subscribes to quotes, trades, bars, and optionally order book deltas
    /// for the configured venue, routing all received data to the matching engines.
    fn register_message_handlers(&self) {
        ...
        // Optionally subscribe to order book deltas if orderbook_execution is enabled
        let (deltas_pattern, deltas_handler) = if self.config.orderbook_execution {
            ...
            let pattern: MStr<Pattern> = format!("data.book.deltas.{venue}.*").into();
            msgbus::subscribe_book_deltas(pattern, handler.clone(), Some(10));
            log::info!("Sandbox subscribed to order book deltas for venue={}", venue);
    ...
    fn deregister_message_handlers(&self) {
        if let Some(handlers) = self.handlers.borrow_mut().take() {
            ...
            // Unsubscribe from order book deltas if subscribed
            if let (Some(pattern), Some(handler)) =
                (handlers.deltas_pattern, handlers.deltas_handler)
            {
