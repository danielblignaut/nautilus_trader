...
pub struct SandboxExecutionClientConfig {
    ...
    /// If order book deltas should be processed by the matching engine.
    pub orderbook_execution: bool,
    /// If orders consume L2 liquidity when matched (realistic slippage).
    /// When true, market orders walk the book and consume depth.
...
impl SandboxExecutionClientConfig {
    ...
    /// Sets whether orderbook execution is enabled.
    #[must_use]
    pub fn with_orderbook_execution(mut self, enabled: bool) -> Self {
        self.orderbook_execution = enabled;
        self
    }

    /// Sets whether liquidity consumption is enabled (L2 realistic fills).
    #[must_use]
    pub fn with_liquidity_consumption(mut self, enabled: bool) -> Self {
        self.liquidity_consumption = enabled;
        self
    }
}
...
mod pyo3_impl {
    ...
    impl SandboxExecutionClientConfig {
        #[new]
        #[pyo3(signature = (venue, starting_balances, trader_id=None, account_id=None, base_currency=None, oms_type=None, account_type=None, default_leverage=None, book_type=None, frozen_account=false, bar_execution=true, trade_execution=false, orderbook_execution=false, liquidity_consumption=false, reject_stop_orders=true, support_gtd_orders=true, support_contingent_orders=true, use_position_ids=true, use_random_ids=false, use_reduce_only=true))]
        ...
        #[getter]
        fn orderbook_execution(&self) -> bool {
            self.orderbook_execution
        }
