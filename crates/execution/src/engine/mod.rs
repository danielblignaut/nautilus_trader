...
impl ExecutionEngine {
    ...
    ///
    /// Uses a deferred event queue to avoid RefCell re-borrow panics when
    /// execution clients (e.g. SandboxExecutionClient) send order events
    /// synchronously from within `execute()`. Events that arrive while the
    /// engine is already borrowed are queued and drained after execute returns.
    pub fn register_msgbus_handlers(engine: Rc<RefCell<Self>>) {
        // Thread-local deferred event queue for re-entrancy safety.
        // When `execute()` holds an immutable borrow and the execution client
        // sends order events synchronously (e.g. sandbox/paper trading),
        // those events are deferred here and processed after execute returns.
        thread_local! {
            static DEFERRED_ORDER_EVENTS: RefCell<VecDeque<OrderEventAny>> =
        ...
        msgbus::register_trading_command_endpoint(
            ...
            TypedIntoHandler::from(move |cmd: TradingCommand| {
                if let Some(rc) = weak1.upgrade() {
                    ...
                    // Drain any order events that were deferred during execute
                    loop {
                        let events: VecDeque<OrderEventAny> = DEFERRED_ORDER_EVENTS
        ...
        msgbus::register_order_event_endpoint(
            ...
            TypedIntoHandler::from(move |event: OrderEventAny| {
                if let Some(rc) = weak2.upgrade() {
                    // Use try_borrow_mut to detect re-entrancy: if the engine
                    // is already borrowed (e.g. by execute()), defer the event.
                    match rc.try_borrow_mut() {
                        Ok(mut eng) => eng.process(event),
                        Err(_) => {
