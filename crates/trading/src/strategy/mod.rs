...
pub trait Strategy: DataActor {
    ...
    /// Instrument IDs this strategy wants to subscribe to for trade data.
    ///
    /// Used by [`BoxedStrategy`] to set up subscriptions on behalf of the inner
    /// trait-object strategy (avoiding the `Self`-type mismatch that occurs when
    /// the inner strategy calls `subscribe_trades` directly).
    ///
    /// Default: empty (strategy manages its own subscriptions in `on_start`).
    fn trade_subscription_ids(&self) -> Vec<InstrumentId> {
        Vec::new()
    }

    /// Instrument IDs this strategy wants order-book delta subscriptions for.
    ///
    /// Same delegation pattern as [`trade_subscription_ids`].
    fn book_subscription_ids(&self) -> Vec<InstrumentId> {
        Vec::new()
    }
